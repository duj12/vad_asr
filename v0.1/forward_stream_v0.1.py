...
class CRNN_VAD_STREAM(torch.nn.Module):
    def __init__(self, model_path, labelencoder_path, output_dim=2, gru_bidirection=False, device='cuda'):
        ...
        self.left_context = 16  #Reception Field=11
        self.central_context = 24
        self.right_centext = 8   #5-Conv2d layers, each layer with 1-frame padding,
        ...
        for chunk_id, start in enumerate(range(0, wav_len, self.chunk_length)):
            ...
            if last_chunk:
                ...
                with torch.no_grad():
                    ...
                    feature = feature.unsqueeze(0)  # add one dimension to match forward
                    vad_chunk, hidden_vector = self.model.forward_stream_vad(feature, hidden_vector)
                    vad_chunk = vad_chunk[0].cpu().numpy()  #single-batch
                    vad_post = np.append(vad_post, vad_chunk, axis=0)
                break
            #frames are enough to generate result, consume frames until they are not enough
            while frames_queue.shape[0] > self.left_context+self.central_context+self.right_centext:
                cur_frame_chunk = frames_queue[:self.left_context+self.central_context+self.right_centext]
                with torch.no_grad():
                    feature = torch.as_tensor(cur_frame_chunk).to(self.device)
                    feature = feature.unsqueeze(0)  #add one dimension to match forward
                    ...
                    vad_chunk = vad_chunk[0].cpu().numpy()  #single-batch
                    vad_post = np.append(vad_post, vad_chunk, axis=0)
                frames_queue = frames_queue[self.left_context+self.central_context:]  #队首central_context之前帧出队
        return vad_post
...
if __name__ == "__main__":
    ...
    labelencoder_path = '../../code/Datadriven-VAD/pretrained_models/labelencoders/students.pth'
