...
import numpy as np
import librosa
import sklearn.preprocessing as pre  #scikit-learn==0.23.0
import sklearn.metrics as skmetrics

def crnn(inputdim=64, outputdim=2, pretrained_from='balanced.pth', **kwargs):
...
class CRNN_VAD_STREAM(torch.nn.Module):
    ...
    '''
    def __init__(self, model_path, labelencoder_path,
                 left_frames=16, central_frames=48, right_frames=16,
                 output_dim=2, gru_bidirection=False, device='cuda'):
        super().__init__()
    ...
    def post_process(self, vad_post, threshold=(0.5, 0.2), min_speech_frames=10):
        ...
        time_stamp = decode_with_timestamps(self.labelencoder, thresholded_prediction)
        # 选出语音时间戳，限制语音段长度超过最小长度
        speech_time_stamp = [[stamp[1], stamp[2]] for stamp in time_stamp
                             if(stamp[0]=='Speech') and (stamp[2]-stamp[1])>min_speech_frames]
...
def decode_with_timestamps(encoder, labels):
...
def vad_postprocess(post_label, off_on_length=20, on_off_length=20, hang_before=10, hang_over=10):
...
def vad_demo():
    ...
    for line in f_wav.readlines():
        ...
        # padding with left and right context
        ...
        for chunk_id, start in enumerate(range(0, wav_padded_len, vad_model.central_context)):
            ...
            #输出的时间戳对应的是中间central_context音频的采样点数
